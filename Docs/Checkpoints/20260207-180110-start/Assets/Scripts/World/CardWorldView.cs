...
public class CardWorldView : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    [SerializeField] private Color shadowColor = new Color(0f, 0f, 0f, 0.14f);
    ...
    [Tooltip("Quando desativado, um clique simples nao deixa carta fixa/flutuando acima da mao.")]
    public bool allowPinOnClick = false;
    ...
    private void BeginDrag(Vector2 screenPos)
    {
        ...
        _tMove?.Kill();
        _tScale?.Kill();
        // Comeca menor enquanto esta proximo da mao
        transform.localScale = _restLocalScale * dragHoldScale;
        _lastLiftAmount = 0f;
    ...
    private void DragTo(Vector2 screenPos)
    {
        ...
        // So cresce quando sai da mao (acima do threshold)
        float liftAbove = Mathf.Max(0f, liftAmount - dragTopLift);
        float t = dragScaleLift > 0f ? Mathf.Clamp01(liftAbove / dragScaleLift) : 0f;
        var startScale = _restLocalScale * dragHoldScale;
        var targetScale = Vector3.Lerp(startScale, _restLocalScale * dragScale, t);
        ...
        // So vai para o topo quando o jogador levanta a carta
        SetSortingOrder(liftAmount > dragTopLift ? order + 1000 : order);
    ...
    private void EndDrag(Vector2 screenPos)
    {
        ...
        float clickMax = Mathf.Max(1f, clickMaxTravelPixels);
        bool isClickGesture = _dragTravelSqr <= (clickMax * clickMax);

...
public class GameBootstrap : MonoBehaviour
{
    ...
    public int worldDrawStackMaxLayers = 12;
    public int worldDrawStackCardsPerLayer = 4;
    public Vector3 worldDrawStackOffset = new Vector3(0.006f, -0.004f, 0.0012f);
    public float worldDrawStackTwist = 1.2f;
    public float worldDrawStackTintStep = 0.01f;

    public bool useWorldDiscardStack = true;
    public int worldDiscardStackMaxLayers = 20;
    public Vector3 worldDiscardStackOffset = new Vector3(0.005f, -0.0035f, 0.0012f);
    public float worldDiscardStackStep = 0.003f;
    public Vector3 worldDiscardOffset = new Vector3(0f, -0.2f, 0f);
    public Vector2 worldDiscardJitter = new Vector2(0.1f, 0.05f);
    public float worldDiscardTwist = 6f;
    public float worldDiscardSettle = 0.15f;
    public float worldDiscardLift = 0.18f;
    public float worldDiscardArc = 0.12f;
    public bool showOnlyTopDiscardCard = true;
    public bool worldDiscardScatter = false;
    public int worldDiscardVisibleStack = 8;
    public float worldDiscardCompactTwist = 2.5f;
    public Vector2 worldDiscardCompactJitter = new Vector2(0.015f, 0.01f);
    ...
    private void InitRuntime()
    {
        ...
        EnsureWorldDiscardShadowVisual();
    ...
    public void DiscardWorldCard(CardWorldView card, Vector3 releaseWorldPos)
    {
        ...
        NormalizeWorldDiscardStackVisuals(card);

        Vector3 target = worldDiscardRoot != null ? worldDiscardRoot.position : releaseWorldPos;
        target += worldDiscardOffset;
        int layer = 0;
        if (!showOnlyTopDiscardCard && useWorldDiscardStack)
        {
            int visibleCap = Mathf.Max(1, worldDiscardVisibleStack);
            layer = Mathf.Clamp(Mathf.Min(_discard.Count - 1, visibleCap - 1), 0, Mathf.Max(0, worldDiscardStackMaxLayers - 1));
            target += worldDiscardStackOffset * layer;
            target.z += worldDiscardStackStep * layer;
        }
        float targetAngle;
        if (showOnlyTopDiscardCard)
        {
            targetAngle = 0f;
        }
        else if (worldDiscardScatter)
        {
            target += new Vector3(
            ...
            targetAngle = Random.Range(-worldDiscardTwist, worldDiscardTwist);
        }
        else
        {
            float side = (layer % 2 == 0) ? -1f : 1f;
            float spread = Mathf.Min(0.02f, layer * 0.0035f);
            target += new Vector3(side * spread, Mathf.Min(0.01f, layer * 0.0015f), 0f);
            target += new Vector3(
                Random.Range(-worldDiscardCompactJitter.x, worldDiscardCompactJitter.x),
                Random.Range(-worldDiscardCompactJitter.y, worldDiscardCompactJitter.y),
                0f
            );
            float t = (layer % 5) / 4f;
            targetAngle = Mathf.Lerp(-Mathf.Abs(worldDiscardCompactTwist), Mathf.Abs(worldDiscardCompactTwist), t);
        }
        ...
        seq.Join(card.transform.DORotateQuaternion(
            Quaternion.Euler(WorldCardTiltX, 0f, targetAngle), 0.18f
        ...
        UpdateWorldDiscardShadow(card, target);
        ApplyWorld();
        UpdatePileVisuals();
    }

    private void NormalizeWorldDiscardStackVisuals(CardWorldView currentTop)
    {
        if (worldDiscardRoot == null) return;

        var views = new List<CardWorldView>(worldDiscardRoot.GetComponentsInChildren<CardWorldView>(true));
        views.RemoveAll(v => v == null || v == currentTop);
        ...
        views.Sort((a, b) => a.transform.GetSiblingIndex().CompareTo(b.transform.GetSiblingIndex()));
        int keepPrevious = Mathf.Max(0, worldDiscardVisibleStack - 1);
        int firstVisible = Mathf.Max(0, views.Count - keepPrevious);
        ...
        for (int i = 0; i < views.Count; i++)
        {
            ...
            int compactIndex = i - firstVisible;
            int layer = Mathf.Clamp(compactIndex, 0, Mathf.Max(0, worldDiscardStackMaxLayers - 1));
            Vector3 pos = worldDiscardRoot.position + worldDiscardOffset;
            if (useWorldDiscardStack)
            {
                pos += worldDiscardStackOffset * layer;
                pos.z += worldDiscardStackStep * layer;
            }

            float zAngle;
            if (worldDiscardScatter)
            {
                pos += new Vector3(
                    Random.Range(-worldDiscardJitter.x, worldDiscardJitter.x),
                    Random.Range(-worldDiscardJitter.y, worldDiscardJitter.y),
                ...
                zAngle = Random.Range(-worldDiscardTwist, worldDiscardTwist);
            }
            else
            {
                float side = (layer % 2 == 0) ? -1f : 1f;
                float spread = Mathf.Min(0.02f, layer * 0.0035f);
                pos += new Vector3(side * spread, Mathf.Min(0.01f, layer * 0.0015f), 0f);
                float t = (layer % 5) / 4f;
                zAngle = Mathf.Lerp(-Mathf.Abs(worldDiscardCompactTwist), Mathf.Abs(worldDiscardCompactTwist), t);
            }

            view.transform.DOKill();
            view.transform.position = pos;
            view.transform.rotation = Quaternion.Euler(WorldCardTiltX, 0f, zAngle);
    ...
    private void EnsureWorldDrawPileVisual()
    {
        ...
        while (_worldDrawStack.Count < layers)
        {
        ...
        for (int i = 0; i < _worldDrawStack.Count; i++)
        {
            ...
            bool active = i < layers && showWorldDrawPile;
            ...
            sr.transform.localPosition = worldDrawPileOffset + (worldDrawStackOffset * i);
        ...
        if (_worldDrawShadow != null)
        {
            bool showShadow = showWorldDrawPile && _back != null;
            _worldDrawShadow.gameObject.SetActive(showShadow);
            if (showShadow)
            {
                _worldDrawShadow.sprite = _back;
                _worldDrawShadow.color = new Color(0f, 0f, 0f, 0.14f);
                _worldDrawShadow.sortingOrder = GetWorldSortingOrderForIndex(0) - 3;
                _worldDrawShadow.transform.localPosition = worldDrawPileOffset + new Vector3(0.03f, -0.035f, 0f);
                _worldDrawShadow.transform.localRotation = Quaternion.identity;
                _worldDrawShadow.transform.localScale = new Vector3(0.92f, 0.78f, 1f);
            }
        }
        ...
        if (_worldDrawStack.Count > 0 && _worldDrawStack[0].sprite != null)
        {
            ...
            collider.size = new Vector3(b.size.x, b.size.y, 0.1f);
        }
    }

    private void EnsureWorldDiscardShadowVisual()
    {
        if (!useWorldCards || worldDiscardRoot == null) return;
        if (_worldDiscardShadow == null)
        {
            ...
            if (_worldDiscardShadow == null)
            {
                var go = new GameObject("DiscardTopShadow");
                go.transform.SetParent(worldDiscardRoot, false);
                _worldDiscardShadow = go.AddComponent<SpriteRenderer>();
            }
        }

        if (_worldDiscardShadow != null)
            _worldDiscardShadow.gameObject.SetActive(_discard.Count > 0);
    }
    ...
    private void UpdateWorldDiscardShadow(CardWorldView topCard, Vector3 topCardTarget)
    {
        if (!useWorldCards || worldDiscardRoot == null) return;
        EnsureWorldDiscardShadowVisual();
        if (_worldDiscardShadow == null) return;

        Sprite topSprite = null;
        if (topCard != null && topCard.spriteRenderer != null)
            topSprite = topCard.spriteRenderer.sprite;
        if (topSprite == null)
            topSprite = TopDiscardSprite() ?? _back;

        _worldDiscardShadow.sprite = topSprite;
        _worldDiscardShadow.color = new Color(0f, 0f, 0f, 0.16f);
        _worldDiscardShadow.sortingOrder = topCard != null
            ? topCard.GetSortingOrder() - 1
            : GetWorldSortingOrderForIndex(0) - 2;

        Vector3 p = topCardTarget + new Vector3(0.03f, -0.04f, 0f);
        p.z = WorldShadowPlaneZ - 0.002f;
        _worldDiscardShadow.transform.position = p;
        _worldDiscardShadow.transform.rotation = Quaternion.identity;
        _worldDiscardShadow.transform.localScale = new Vector3(0.92f, 0.78f, 1f);
        _worldDiscardShadow.gameObject.SetActive(true);
    }
